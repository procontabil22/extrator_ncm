"""
Microserviço de Extração Fiscal v3.15
=====================================
Três camadas: Determinístico → Regex → Semântico (LLM)
Mapeado para a estrutura REAL das tabelas no Supabase.

v3.15 — Correção whitelist de normas e regex (2026-03-22):
        • CORRIGIDO: normas fantasma "Lei 10.485/2003" ... "Lei 10.485/2026"
          ainda eram geradas pelo XLSX porque o campo Legislação contém textos
          como "Lei 10.485/2002, art. 2º, I; art. 3º" e o regex capturava
          "2", "3", "I" como anos dentro do intervalo 1988-2026 (ex: "art.
          2003º" → ano 2003 passava no filtro). Solução definitiva: substituir
          o regex livre por uma WHITELIST de leis monofásicas federais válidas.
          _extrair_normas_de_base_legal() agora usa _WHITELIST_NORMAS_MONO —
          conjunto fixo das leis/decretos/MPs que realmente regem PIS/COFINS
          monofásico. Qualquer combinação número/ano fora da whitelist é
          descartada silenciosamente.
        • CORRIGIDO: regex RE_LEI_MONO agora aceita "Lei10.147/2000" (sem
          espaço após "Lei") para capturar o NCM 30039056 cujo base_legal
          é "Exclusão expressa Lei10.147/2000".
        • ADICIONADO: SQL de limpeza das normas fantasma já inseridas —
          ver fix_normas_fantasma_v3.15.sql.
...
app = FastAPI(title="Extrator Fiscal v3.15", version="3.15.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# v3.15: \s* (zero ou mais espaços) para cobrir "Lei10.147/2000" sem espaço
...
# =============================================================================
# v3.15 — WHITELIST DE NORMAS MONOFÁSICAS PIS/COFINS VÁLIDAS
# Fonte: Portal SPED / RFB — tabelas 4.3.10, 4.3.11, 4.3.13 EFD-Contribuições
# Chave: "tipo_norma|numero|ano" — apenas combinações realmente existentes
# são aceitas; qualquer resultado de regex fora desta lista é descartado.
# =============================================================================
_WHITELIST_NORMAS_MONO: frozenset = frozenset({
    # Leis federais
    "Lei|9.718|1998",
    "Lei|10.147|2000",
    "Lei|10.336|2001",
    ...
    # Medidas Provisórias
    "Medida Provisória|1.157|2023",
    "Medida Provisória|1.163|2023",
    # Decretos
    "Decreto|5.059|2004",
    "Decreto|5.171|2004",
    "Decreto|6.573|2008",
...
def _extrair_normas_de_base_legal(base_legal: str) -> list[dict]:
    ...
    def _registrar(tipo, numero, ano_str, trecho):
        ...
        # v3.15: valida contra whitelist — descarta normas fantasma
        chave_wl = f"{tipo}|{numero}|{ano}"
        if chave_wl not in _WHITELIST_NORMAS_MONO:
            log.debug(f"[normas-mono] descartado fora da whitelist: {chave_wl}")
            return
